...
const Card = ({ ...props }) => {
  ...
  for (let i = 0; i < 5; i++) {
    if (i < data.rating) {
      ratingNodes.push(
        <img src={ActiveStar} alt="active rating" key={i + 1} />
      );
    } else {
      ratingNodes.push(
        <img src={DisabledStar} alt="disabled rating" key={i + 1} />
      );
    }
  }
  ...
  return (
    <div className={`Card ${highlight ? "Active" : ""}`}>
      ...
      <div className="Item" style={{ marginBottom: "8px" }}>
        {ratingNodes}
      </div>
      <div className="Item">
      ...
      <div className="Item">
        <span>
          <img src={CompensationIcon} alt="compensation" />
          {data.compensation}
        </span>
        <span>
      ...
      <div className="Item">
        <img src={PostedOnIcon} alt="posted on" />
        {data.postedOn}
      </div>
